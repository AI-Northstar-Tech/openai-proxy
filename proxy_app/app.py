...
@app.route('/')
def appEntry():
    return '<h2>Welcome to the OpenAI API Proxy Server</h2>', 200, {'ContentType':'text/html'}
...
def handleChatCompetion(proxy_key):
    ...
    if(db.validate_api_key(proxy_key)):
        is_valid, rem_quota, req_id = db.validate_api_key_request(proxy_key, tiktoken_tokens_cost)
    else:
        is_valid = False
        rem_quota = None
    ...
    if(is_valid):
        ...
        if(is_valid_resp and not stream):
            ...
            print(f"Total Token Count: {tokens_used}, Actual Total Token Cost: {tokens_cost}")
            ...
            def generate_stream():
                ...
                try:
                    req_data['max_tokens'] = min(int(req_data['max_tokens']), MAX_TOKEN_LIMIT)
                except KeyError:
        ...
        if(rem_quota != None):
            response = Response(f'Insufficient Quota, Remaining Quota: {rem_quota} and Request Cost: {tiktoken_tokens_cost}', mimetype='application/json')
        else:
...
def handleEmbedding(proxy_key):
    ...
    if(is_valid):
        ...
        if(is_valid_resp):
            ...
            print(f"Actual Total Token Count: {tokens_used}, Actual Total Token Cost: {tokens_cost}")
            db.insert_data(req_id, proxy_key, req_data, response)
            db.update_remQuota(proxy_key, rem_quota-tokens_cost)
            print(f"Remaining Quota: {rem_quota-tokens_cost}")
