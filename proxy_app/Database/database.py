...
class ProxyAPIDatabase():
    ...
    def create_api_key_with_quota(self, api_key, rem_quota, req_count):
        with Session(self.engine) as session:
            api_key_to_quota = APIKeyToQuota(api_key=api_key, rem_quota=rem_quota, req_count=req_count)
    ...
    def insert_data(self, req_id, api_key, req_data, response):
        with Session(self.engine) as session:
            requestResponse = OpenAIRequestResponse(req_id=req_id, api_key=api_key, req_data=req_data, response=response)
    ...
    def update_remQuota(self, api_key, rem_quota):
        with Session(self.engine) as session:
            result = session.query(APIKeyToQuota).filter(APIKeyToQuota.api_key == api_key).first()
            if(result):
    ...
    def validate_api_key(self, api_key):
        with Session(self.engine) as session:
            api_key_to_quota = session.query(APIKeyToQuota).filter(APIKeyToQuota.api_key == api_key).first()
    ...
    def get_AllRequestData(self, api_key):
        with Session(self.engine) as session:
            query_list = session.query(OpenAIRequestResponse).filter(OpenAIRequestResponse.api_key == api_key).all()
